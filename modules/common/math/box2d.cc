...
void Box2d::InitCorners() {
  ...
  corners_.clear();
  corners_.emplace_back(center_.x() + dx1 + dx2, center_.y() + dy1 + dy2);   // 车辆右上角的坐标
  corners_.emplace_back(center_.x() + dx1 - dx2, center_.y() + dy1 - dy2);     // 车辆左上角的坐标
  corners_.emplace_back(center_.x() - dx1 - dx2, center_.y() - dy1 - dy2);      // 车辆左下角的坐标
  corners_.emplace_back(center_.x() - dx1 + dx2, center_.y() - dy1 + dy2);    // 车辆右下角的坐标

  for (auto &corner : corners_) {   // 得到在x轴y轴投影的最大最小坐标
...
bool Box2d::HasOverlap(const Box2d &box) const {
  if (box.max_x() < min_x() || box.min_x() > max_x() || box.max_y() < min_y() ||
      box.min_y() > max_y()) {           // 快速剔除一部分非碰撞Box
    return false;
  }

  const double shift_x = box.center_x() - center_.x();            // 障碍物与本车中心点距离在x轴上的投影
  const double shift_y = box.center_y() - center_.y();            // 障碍物与本车中心点距离在y轴上的投影

  const double dx1 = cos_heading_ * half_length_;   // 本车的
  const double dy1 = sin_heading_ * half_length_;
  const double dx2 = sin_heading_ * half_width_;
  const double dy2 = -cos_heading_ * half_width_;
  const double dx3 = box.cos_heading() * box.half_length();  // 障碍物的
  const double dy3 = box.sin_heading() * box.half_length();
  const double dx4 = box.sin_heading() * box.half_width();
  const double dy4 = -box.cos_heading() * box.half_width();

//!  只要两个物体不发生碰撞，则两个半轴的投影（在某一投影轴上）
//!  必然  '存在'  两个物体中心距离在投影轴上的投影长L
//! 大于两个物体半轴在投影轴上的投影长度的和
  return std::abs(shift_x * cos_heading_ + shift_y * sin_heading_) <=     // 在本车的长轴上投影
             std::abs(dx3 * cos_heading_ + dy3 * sin_heading_) +
                 std::abs(dx4 * cos_heading_ + dy4 * sin_heading_) +             // todo //  障碍物半轴在本车长轴上的投影
                 half_length_ &&
         std::abs(shift_x * sin_heading_ - shift_y * cos_heading_) <=       // 在本车的横轴上投影
             std::abs(dx3 * sin_heading_ - dy3 * cos_heading_) +
                 std::abs(dx4 * sin_heading_ - dy4 * cos_heading_) +            // todo //  障碍物半轴在本车横轴上的投影       
                 half_width_ &&
         std::abs(shift_x * box.cos_heading() + shift_y * box.sin_heading()) <=                // 在障碍物的长轴上进行投影
             std::abs(dx1 * box.cos_heading() + dy1 * box.sin_heading()) +
                 std::abs(dx2 * box.cos_heading() + dy2 * box.sin_heading()) +                 // todo //  本车半轴在本车长轴上的投影
                 box.half_length() &&
         std::abs(shift_x * box.sin_heading() - shift_y * box.cos_heading()) <=             // 在障碍物的横轴上进行投影
             std::abs(dx1 * box.sin_heading() - dy1 * box.cos_heading()) +
                 std::abs(dx2 * box.sin_heading() - dy2 * box.cos_heading()) +              // todo //  本车半轴在本车横轴上的投影
                 box.half_width();
}
