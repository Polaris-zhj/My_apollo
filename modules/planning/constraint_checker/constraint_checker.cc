...
ConstraintChecker::Result ConstraintChecker::ValidTrajectory(
  ...
  for (const auto& p : trajectory) {
    ...
    double lon_a = p.a();    // 对轨迹中的纵向加速度进行检查
    ...
    double kappa = p.path_point().kappa();     // 对轨迹中的曲率进行检查
  ...
  for (size_t i = 1; i < trajectory.NumOfPoints(); ++i) {
    ...
    if (!WithinRange(lon_jerk, FLAGS_longitudinal_jerk_lower_bound,
                     FLAGS_longitudinal_jerk_upper_bound)) {        // 对轨迹中的加速度变化率进行检查
    ...
    double lat_a = p1.v() * p1.v() * p1.path_point().kappa(); // 对轨迹中的横向加速度进行检查
