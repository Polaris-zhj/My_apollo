...
bool CollisionChecker::InCollision(
  ...
  const auto& vehicle_config =
      common::VehicleConfigHelper::Instance()->GetConfig();  // 获取自车信息
  ...
  for (size_t i = 0; i < discretized_trajectory.NumOfPoints(); ++i) {
    ...
    double shift_distance =
        ego_length / 2.0 - vehicle_config.vehicle_param().back_edge_to_center();   // 车辆中心与几何中心不重合，计算偏移距离
    Vec2d shift_vec{shift_distance * std::cos(ego_theta), 
                    shift_distance * std::sin(ego_theta)};   // 偏移向量
    ego_box.Shift(shift_vec);

    for (const auto& obstacle_box : predicted_bounding_rectangles_[i]) {
      if (ego_box.HasOverlap(obstacle_box)) {     // 查看本车的box与障碍物的box是否发生重叠
        return true;
      }
    }
  }
...
void CollisionChecker::BuildPredictedEnvironment(
  ...
  bool ego_vehicle_in_lane = IsEgoVehicleInLane(ego_vehicle_s, ego_vehicle_d);  //车辆是否在车道内
  std::vector<const Obstacle*> obstacles_considered;
  for (const Obstacle* obstacle : obstacles) {
    if (obstacle->IsVirtual()) {   // 忽略掉：虚拟障碍物
      continue;
    }
    if (ego_vehicle_in_lane &&
        (IsObstacleBehindEgoVehicle(obstacle, ego_vehicle_s,
                                    discretized_reference_line) ||
         !ptr_path_time_graph_->IsObstacleInGraph(obstacle->Id()))) {  // 忽略掉：车辆在车道内并且（障碍物在本车后面 || ST上没有此障碍物投影（即障碍物行驶轨迹不会与本车轨迹发生重合））
      continue;
    }
  ...
  while (relative_time < FLAGS_trajectory_time_length) {
    ...
    for (const Obstacle* obstacle : obstacles_considered) {
      ...
      TrajectoryPoint point = obstacle->GetPointAtTime(relative_time);    // 计算relative_time时障碍物的位置
      Box2d box = obstacle->GetBoundingBox(point);  // 获取relative_time时障碍物位置处的BOX
      box.LongitudinalExtend(2.0 * FLAGS_lon_collision_buffer);  // 纵向延伸2
      box.LateralExtend(2.0 * FLAGS_lat_collision_buffer);     // 横向延伸0.1
      predicted_env.push_back(std::move(box));  // 将此时刻box全部压入predicted_env中
    }
    predicted_bounding_rectangles_.push_back(std::move(predicted_env));  // 将每一时刻内的predicted_env压入
