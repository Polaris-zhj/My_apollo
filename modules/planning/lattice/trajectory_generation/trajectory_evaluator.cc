...
TrajectoryEvaluator::TrajectoryEvaluator(
  ...
  path_time_intervals_ = path_time_graph_->GetPathBlockingIntervals(
      start_time, end_time, FLAGS_trajectory_time_resolution);  // 得到每一时刻所有障碍物s的上下界限

  reference_s_dot_ = ComputeLongitudinalGuideVelocity(planning_target);  // 计算每一时刻参考速度
  ...
  for (const auto& lon_trajectory : lon_trajectories) {
    ...
    if (!ConstraintChecker1d::IsValidLongitudinalTrajectory(*lon_trajectory)) {  // todo 判断纵向轨迹是否合理
      continue;
    }
...
double TrajectoryEvaluator::Evaluate(
  ...
  double lon_objective_cost =
      LonObjectiveCost(lon_trajectory, planning_target, reference_s_dot_);    // 错失目标代价

  double lon_jerk_cost = LonComfortCost(lon_trajectory);

  double lon_collision_cost = LonCollisionCost(lon_trajectory);  // 纵向碰撞代价
  ...
  double evaluation_horizon =
      std::min(FLAGS_speed_lon_decision_horizon,
               lon_trajectory->Evaluate(0, lon_trajectory->ParamLength()));      // 轨迹中的纵向延伸距离
  ...
  // Lateral costs
  double lat_offset_cost = LatOffsetCost(lat_trajectory, s_values);        // 横向偏移代价

  double lat_comfort_cost = LatComfortCost(lon_trajectory, lat_trajectory);   // 横向加速度与横向加速度变化率和的最大值
...
double TrajectoryEvaluator::LatOffsetCost(
  ...
  for (const auto& s : s_values) {
    ...
    if (lat_offset * lat_offset_start < 0.0) {
      cost_sqr_sum += cost * cost * FLAGS_weight_opposite_side_offset;     // 反向偏离代价
